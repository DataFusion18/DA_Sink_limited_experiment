...
q=0 # Case 0
Cday.data.set = subset(Cday.data.processed,volume==5) # Consider the 5L seedling to test the parameter sensitivity
...
q=4 # Case 5
param.pot = subset(summary.param,(volume.group %in% 1 & variable %in% c("k","sf","af","as","ar")))
param.free = subset(summary.param,(volume.group %in% 3 & variable %in% c("Y")))
param = rbind(param.pot, param.free)
keeps = c("Date", "variable", "Parameter")
param = param[ , keeps, drop = FALSE]
param.casted = dcast( param , Date ~ variable )
...
q=5 # Case 6
param.pot = subset(summary.param,(volume.group %in% 1 & variable %in% c("Y","af","as","ar","k")))
param.free = subset(summary.param,(volume.group %in% 3 & variable %in% c("sf")))
param = rbind(param.pot, param.free)
keeps = c("Date", "variable", "Parameter")
param = param[ , keeps, drop = FALSE]
param.casted = dcast( param , Date ~ variable )
...
q=6 # Case 7
param.pot = subset(summary.param,(volume.group %in% 1 & variable %in% c("Y","af","as","ar","sf")))
param.free = subset(summary.param,(volume.group %in% 3 & variable %in% c("k")))
param = rbind(param.pot, param.free)
...
png("output/Figure_6_parameter_shifting_individual_param.png", units="px", width=2000, height=3000, res=220)
...
#-------------------------------------------------------------------------------------
q=0 # Case 8
Cday.data.set = subset(Cday.data.processed,volume==1000)
...
mass.quantify[9,c(1:3)] = subset(shift.output, Date %in% as.Date("2013-05-21") & Case %in% q & variable %in% as.factor(c("Mleaf","Mstem","Mroot")))$value
rownames(mass.quantify)[9] = "FreeSeedling"
mass.quantify[9,4] = sum(mass.quantify[9,c(1:3)])
mass.quantify[10,] = mass.quantify[9,] - mass.quantify[8,]
rownames(mass.quantify)[10] = "Interactions"

#-------------------------------------------------------------------------------------

# mass.quantify = mass.quantify[,-c(2:8)]
write.csv(mass.quantify, file = "output/final_mass_changes_individual_param.csv", row.names = FALSE)
#-------------------------------------------------------------------------------------
